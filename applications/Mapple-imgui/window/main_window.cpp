...
namespace easy3d {
    ...
    void MainWindow::draw_overlay(bool* visible)
    {
        ...
        if (ImGui::Begin("Mapple: Information", visible, (corner != -1 ? ImGuiWindowFlags_NoMove : 0) | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize/* | ImGuiWindowFlags_AlwaysAutoResize*/ | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav))
        {
    ...
    void MainWindow::draw_menu_view() {
        if (ImGui::BeginMenu("View"))
        {
            ...
            ImGui::Separator();
            if (ImGui::MenuItem("Save Camera State", nullptr))
                std::cout << "save camera state" << std::endl;
            if (ImGui::MenuItem("Load Camera State", nullptr))
            ...
            if (!panels_.empty()) {
                ...
                for (std::size_t i = 0; i < panels_.size(); ++i) {
                    Panel* win = panels_[i];
                    ImGui::MenuItem(win->name_.c_str(), nullptr, &win->visible_);
                }
            }
    ...
    void MainWindow::draw_menu_select() {
        if (ImGui::BeginMenu("Select"))
        {
            ...
            ImGui::Separator();
            ImGui::MenuItem("Invert", nullptr, false);
            ImGui::MenuItem("Delete", nullptr, false);
            ImGui::EndMenu();
        }
    }
