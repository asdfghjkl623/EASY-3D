...
void ProgressLogger::update(bool show_text) {
	std::size_t percent = cur_val_ * 100 / mpl_max<std::size_t>(1, max_val_ - 1);
	if(percent != cur_percent_) {
		cur_percent_ = percent ;
		if(!quiet_) {
			Progress::instance()->notify(mpl_min<std::size_t>(cur_percent_, 100), show_text);
		}
	}
}
