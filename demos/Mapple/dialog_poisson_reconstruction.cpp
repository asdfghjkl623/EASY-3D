...
void DialogPoissonReconstruction::reconstruct() {
    ...
    if (cloud) {
        ...
        if (mesh) {
            std::string name = file::dir_name(cloud->name()) + "/" + file::base_name(cloud->name()) + "_poisson_reconstruction.ply";
            mesh->set_name(name);
...
void DialogPoissonReconstruction::trim() {
    ...
    if (mesh) {
        ...
        if (trimmed_mesh) {
            std::string name = file::dir_name(mesh->name()) + "/" + file::base_name(mesh->name()) + "_trimmed.ply";
